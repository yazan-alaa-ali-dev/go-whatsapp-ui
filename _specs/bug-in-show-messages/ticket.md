...
title: bug in show messages
mode: standard           # single workflow form — no other modes (ADR-009)
state: verified          # AUTHORITATIVE workflow state — set by OWNER WAIVER, not by a passing /verify (see verify.md §9)
status: active           # orthogonal health flag: active | blocked
owner: developer         # accountable role/person (em | developer | ai_agent | name)
created_at: 2026-08-01   # YYYY-MM-DD
updated_at: 2026-08-02   # YYYY-MM-DD (bumped on every state change)
...
# Ticket Record — bug-in-show-messages
...
# NOT a passing /verify. No acceptance criterion was executed; the owner waived
# verification to unblock /publish-pr (PB-1). See verify.md §9.
- state: verified
